...
class _StuDashboardState extends State<StuDashboard> {
  ...
  void closeProfileMenu() {
    if (showProfileMenu) setState(() => showProfileMenu = false);
  }
  ...
  Future<void> _loadProfile() async {
    try {
      final data = await ApiService.get("/student/profile/"); // Backend route
      setState(() => profileData = data);
      
      // Fetch profile image
      if (data["user_id"] != null) {
        await _getProfileImage(data["user_id"]);
      }
    } catch (e) {
  ...
  Future<void> _getProfileImage(String userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        setState(() => profileImage = response.bodyBytes);
      }
    } catch (e) {
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Text("${profileData?['name'] ?? 'Student 👋'}", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                            ],
                          ),
                          GestureDetector(
                            ...
                            child: CircleAvatar(
                              ...
                              child: profileImage == null 
                                ? const Icon(Icons.person, color: Color(0xFF2D9EE0))
                                : null,
                            ),
                          ),
                        ],
                      ...
                      GridView.count(
                        ...
                        mainAxisSpacing: 18,
                        children: [
                          _buildMenuCard(Icons.person, "Profile Overview", () => Navigator.pushNamed(context, '/stu_profile')),
                          _buildMenuCard(
                            Icons.assignment,
                            "Quiz Result",
                            () {
                              ...
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => StuQuizsummary(userId: userId),
                                ),
                              );
                            },
                          ),
                          ...
                          _buildMenuCard(Icons.task, "AI Task", () => Navigator.pushNamed(context, '/ai_task')),
                    ...
                    child: Material(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text("👤 ${profileData?['name'] ?? 'Student'}", style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF091E3A))),
                            const SizedBox(height: 6),
                            Text("✉ ${profileData?['email'] ?? 'student@example.com'}", style: const TextStyle(color: Colors.black54)),
                            const Divider(),
